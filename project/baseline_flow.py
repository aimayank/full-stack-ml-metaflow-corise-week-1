...
class BaselineNLPFlow(FlowSpec):
    ...
    def baseline(self):
        ...
        self.valdf["baseline_pred"] = 1
        self.base_acc = accuracy_score(self.valdf["label"],self.valdf["baseline_pred"])
        self.base_rocauc = roc_auc_score(self.valdf["label"],self.valdf["baseline_pred"])
        self.next(self.end)
    ...
    def end(self):
        ...
        fp = self.valdf.loc[(self.valdf['baseline_pred'] == 1) & (self.valdf['label'] == 0)]
        current.card.append(Table.from_dataframe(fp))
        
        current.card.append(Markdown("## Examples of False Negatives"))
        # TODO: compute the false positive predictions where the baseline is 0 and the valdf label is 1. 
        # TODO: display the false_negatives dataframe using metaflow.cards
        fn = self.valdf.loc[(self.valdf['baseline_pred'] == 0) & (self.valdf['label'] == 1)]
